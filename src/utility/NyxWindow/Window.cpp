...
namespace nyx
{
  namespace vkg
  {
    ...
    struct NyxWindowData
    {
      ...
      bool              resizable     ; ///< Whether or not this window is resizable.
    ...
    void NyxWindowData::setMouseCapture( bool capture )
    {
      ...
      if( this->initialized )
      {
        Impl::setWindowMouseCapture( this->id, capture ) ;
      }
    }
    ...
    {
      this->id = id ;
      Impl::addWindow( this->id, this->title.c_str(), this->width, this->height ) ;
    }
    ...
    void Window::initialize()
    {
      ...
      {
        Impl::setWindowResizable   ( data().id, data().resizable     ) ;
        Impl::setWindowTitle       ( data().id, data().title.c_str() ) ;
        Impl::setWindowMouseCapture( data().id, data().capture_mouse ) ;
        Impl::setWindowWidth       ( data().id, data().width         ) ;
        Impl::setWindowHeight      ( data().id, data().height        ) ;
      }
